...
test('Arc materializer deletes stale generated arc-builder subagent during materialization', async () => {
  const mod = await import('../extensions/arc/subagents.ts');
  const markers = [
    mod.ARC_SUBAGENT_GENERATED_MARKER,
  ...
  for (const marker of markers) {
    ...
    try {
      ...
      const targetDir = mod.resolveArcSubagentDir('user', cwd, homeDir);
      await mkdir(targetDir, { recursive: true });
      await writeFile(path.join(targetDir, 'arc-builder.md'), `${marker}\nlegacy`, 'utf8');
      ...
      await assert.rejects(readFile(path.join(targetDir, 'arc-builder.md'), 'utf8'));
      const cleanup = result.writes.find((entry) => entry.agent === 'arc-builder');
      assert.equal(cleanup?.status, 'written');
      assert.match(cleanup?.reason ?? '', /removed stale generated subagent/i);
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  }
...
test('Arc materializer cleans stale arc-builder in modern and legacy user dirs without deleting custom files', async () => {
  const mod = await import('../extensions/arc/subagents.ts');
  const root = await mkdtemp(path.join(tmpdir(), 'arc-subagents-'));
  try {
    const cwd = path.join(root, 'project');
    ...
    const targetDir = mod.resolveArcSubagentDir('user', cwd, homeDir);
    const legacyDir = mod.resolveArcSubagentDir('user', cwd, homeDir, { legacyUserDir: true });
    await mkdir(targetDir, { recursive: true });
    await mkdir(legacyDir, { recursive: true });
    await writeFile(path.join(targetDir, 'arc-builder.md'), '# my custom specialist', 'utf8');
    await writeFile(path.join(legacyDir, 'arc-builder.md'), `${mod.ARC_SUBAGENT_GENERATED_MARKER}\nlegacy`, 'utf8');
    ...
    assert.equal(await readFile(path.join(targetDir, 'arc-builder.md'), 'utf8'), '# my custom specialist');
    await assert.rejects(readFile(path.join(legacyDir, 'arc-builder.md'), 'utf8'));

    const cleanups = result.writes.filter((entry) => entry.agent === 'arc-builder');
    assert.equal(cleanups.find((entry) => entry.target === path.join(targetDir, 'arc-builder.md'))?.status, 'skipped');
    assert.equal(cleanups.find((entry) => entry.target === path.join(legacyDir, 'arc-builder.md'))?.status, 'written');
    assert.match(cleanups.find((entry) => entry.target === path.join(targetDir, 'arc-builder.md'))?.reason ?? '', /preserving custom stale subagent/i);
    assert.match(cleanups.find((entry) => entry.target === path.join(legacyDir, 'arc-builder.md'))?.reason ?? '', /removed stale generated subagent/i);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
