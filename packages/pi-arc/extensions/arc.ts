...
type ArcAgentName = "coder" | "devops" | "code-reviewer" | "doc-writer" | "evaluator" | "issue-manager" | "spec-reviewer";
...
const ARC_PROFILE_RECOMMENDATIONS: Record<ArcModelProfileKey, ArcProfileRecommendation> = {
  ...
  devops: { modelId: "gpt-5.5", thinking: "high", reason: "infrastructure risk and operational safety" },
...
export default function arcExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    label: "Arc Agent",
    description:
      "Run a bundled Arc specialist agent (coder, devops, reviewer, issue-manager, etc.) in a fresh Pi subprocess. Output is truncated to 50KB/2000 lines.",
    promptSnippet: "Delegate Arc issue-management, implementation, devops, review, docs, and evaluation tasks to bundled specialist agents.",
    promptGuidelines: [
      "Prefer true pi-subagents Arc specialists (arc-coder, arc-devops, arc-issue-manager, arc-code-reviewer, etc.) when available/auto-materialized so long runs can be monitored with /subagents-status.",
  ...
  pi.registerCommand("arc-subagents-sync", {
    description: "Repair generated Arc specialist definitions (arc-coder, arc-devops, arc-doc-writer, arc-spec-reviewer, arc-code-reviewer, arc-evaluator, arc-issue-manager) in user/project scope",
