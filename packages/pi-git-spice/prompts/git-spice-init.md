...
4. After init, run `git-spice --no-prompt auth status` and report whether the user is logged in. If not, suggest `git-spice auth login` — terminal-only; do not execute via a Pi/tool subprocess.
